-- WHERE 조건절
SELECT * FROM EMPLOYEES WHERE JOB_ID = 'IT_FROG';
SELECT FIRST_NAME, JOB_ID FROM EMPLOYEES WHERE FIRST_NAME = 'David';
SELECT * FROM EMPLOYEES WHERE SALARY >= 15000;
SELECT * FROM EMPLOYEES WHERE DEPARTMENT_ID <> 90; -- 같지 않다
SELECT * FROM EMPLOYEES WHERE HIRE_DATE = '06/03/07'; -- 날짜 비교도 문자열로 한다
SELECT * FROM EMPLOYEES WHERE HIRE_DATE >= '06/03/01'; -- 날짜도 대소비교 가능

-- BETWEEN AND 연산자 ~사이에
SELECT * FROM EMPLOYEES WHERE SALARY BETWEEN 5000 AND 10000; -- 이상 ~ 이하
SELECT * FROM EMPLOYEES WHERE HIRE_DATE BETWEEN '03/01/01' AND '03/12/31';

-- IN 연산자
SELECT * FROM EMPLOYEES WHERE DEPARTMENT_ID IN (50, 60, 70);
SELECT * FROM EMPLOYEES WHERE JOB_ID IN ('IT_FROG', 'ST_MAN');

-- LIKE 연산자는 검색에 사용되며 리터럴문자 % _
SELECT * FROM EMPLOYEES WHERE HIRE_DATE LIKE '03%'; -- 03으로 시작하는
SELECT * FROM EMPLOYEES WHERE HIRE_DATE LIKE '%03'; -- 03으로 끝나는
SELECT * FROM EMPLOYEES WHERE HIRE_DATE LIKE '%03%'; -- 03이 포함된 HIRE_DATE 보여줌
SELECT * FROM EMPLOYEES WHERE JOB_ID LIKE '%MAN%'; -- MAN이 들어간 JOB_ID

SELECT * FROM EMPLOYEES WHERE FIRST_NAME LIKE '_a%'; -- FIRST_NAME의 2번째 글자가 a인 사람
SELECT * FROM EMPLOYEES WHERE FIRST_NAME LIKE '__a%'; -- FIRST_NAME의 3번째 글자가 a인 사람

-- NULL값 찾기 IS NULL, IS NOT NULL 
SELECT * FROM EMPLOYEES WHERE COMMISSION_PCT = NULL; -- 데이터가 안 나옴
SELECT * FROM EMPLOYEES WHERE COMMISSION_PCT IS NULL; -- 보너스가 없는 사람
SELECT * FROM EMPLOYEES WHERE COMMISSION_PCT IS NOT NULL; -- 보너스가 있는 사람

-- AND, OR 중에 AND가 더 빠름
SELECT * FROM EMPLOYEES WHERE JOB_ID IN ('IT_PROG', 'FI_MGR');
SELECT * FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG' OR JOB_ID = 'FI_MGR'; -- 위에랑 같음
SELECT * FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG' OR SALARY >= 5000; -- IT PROG이거나 급여가 5000이상인 사람
SELECT * FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG' AND SALARY >= 5000; -- IT PROG인데 급여가 5000이상인 사람

SELECT * FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG' OR JOB_ID = 'FI_MGR' AND SALARY >= 6000; -- AND 먼저 동작하고 OR동작
SELECT * FROM EMPLOYEES WHERE (JOB_ID = 'IT_PROG' OR JOB_ID = 'FI_MGR' ) AND SALARY >= 6000; -- 소괄호 OR가 먼저 동작됨

-- NOT 부정의 의미 -> 연산 키워드와 사용됨
SELECT * FROM EMPLOYEES WHERE DEPARTMENT_ID NOT IN (50, 60); -- 부서 아이디가 50, 60이 아닌
SELECT * FROM EMPLOYEES WHERE JOB_ID NOT LIKE '%MAN%'; -- 직업명이 MAN이 아닌 사람
SELECT * FROM EMPLOYEES WHERE DEPARTMENT_ID NOT BETWEEN 50 AND 60;

-------------------------------------------------------
-- ORDER BY 
SELECT * FROM EMPLYOEES ORDER BY SALARY ASC; -- 샐러리 기준, 아무것도 안 적으면 기본 ASC 오름차순
SELECT * FROM EMPLOYEES ORDER BY DEPARTMENT_ID DESC; -- 내림차순
SELECT FIRST_NAME, SALARY * 12 AS 연봉 FROM EMPLOYEES ORDER BY 연봉; -- 별칭을 ORDER절이 마지막에 동작하기 때문에 사용할 수 있음

-- 정렬을 2개 이상 컬럼으로 시킬 수 있음 
-- 부서번호가 높은 사람들 중에서도 급여가 높은 사람들 기준으로 정렬
SELECT * FROM EMPLOYEES ORDER BY DEPARTMENT_ID DESC, SALARY DESC;

SELECT * FROM EMPLOYEES WHERE JOB_ID IN ('IT_PROG', 'SA_MAN') ORDER BY FIRST_NAME ASC;





