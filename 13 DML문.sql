...
-----------------------------------------------------
-- DELETE구문
-- 트랜잭션이 있긴 하지만 삭제하기 전에 반드시 SELECT문으로 삭제 조건에 해당하는 데이터를 꼭 확인하는 습관이 있어야 함
SELECT * FROM EMPS WHERE EMPLOYEE_ID = 140;
DELETE FROM EMPS WHERE EMPLOYEE_ID = 140; -- KEY를 통해서 지우는 편이 좋음
...
-------------------------------------------------------
-- DELETE문이 실행되는 것은 아니다
-- 테이블이 연관관계(FX) 제약을 가지고 있다면 지워지지 않는다 (참조무결성 제약)
SELECT * FROM EMPLOYEES;
SELECT * FROM DEPARTMENTS;
DELETE FROM DEPARTMENTS WHERE DEPARTMENT_ID = 100;

---------------------------------------------------------
-- MERGE문 (타겟테이블 데이터가 있으면 UPDATE, 없으면 INSERT구문을 수행하는 방법)

SELECT * FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG';

MERGE INTO EMPS A -- 타겟테이블
USING (SELECT * FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG') B -- 합칠 테이블
ON (B.EMPLOYEE_ID = A.EMPLOYEE_ID) -- 연결할 키
...
-- 2ND 서브쿼리절로 다른 테이블을 가져오는 게 아니라, 직접 값을 넣을 때 DUAL을 쓸 수도 있다
MERGE INTO EMPS A
USING DUAL
...
WHEN NOT MATCHED THEN -- 일치하지 않으면
    ...
    VALUES (107, 'HONG', 'EXAMPLES', SYSDATE, 'DBA');
    ROLLBACK;
...
--1. 부서명 영업부를 삭제 하세요
SELECT DEPARTMENT_NAME FROM DEPTS WHERE DEPARTMENT_NAME = '영업';
DELETE FROM DEPTS WHERE DEPARTMENT_NAME = '영업';
--2. 부서명 NOC를 삭제하세요
SELECT DEPARTMENT_NAME FROM DEPTS WHERE DEPARTMENT_NAME = 'NOC';
