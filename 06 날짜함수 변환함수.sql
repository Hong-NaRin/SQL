...
SELECT TO_CHAR(SYSDATE, 'YY"년" MM"월" DD"일"') AS TIME FROM DUAL; -- 포맷값이 아닌 값을 쓰려면 쌍따옴표(" ")로 묶어준다
...
---------------------------------------------------------
-- NULL처리 함수
SELECT NVL(1000, 0), NVL(NULL, 0) FROM DUAL; -- 첫번째 NULL이 아니라서 1000, 두번째 NULL이라 0반환
...
SELECT FIRST_NAME, SALARY, COMMISSION_PCT, SALARY + SALARY*NVL(COMMISSION_PCT, 0) AS 최종급여 FROM EMPLOYEES; -- COMMISSION_PCT이 NULL일 경우 0으로 대체
