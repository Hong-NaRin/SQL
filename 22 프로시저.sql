...
CREATE OR REPLACE PROCEDURE NEW_JOB_PROC -- 이름이 같으면 자동으로 수정됨
    (P_JOB_ID IN VARCHAR2, -- 매개변수 IN 타입
...
-- 프로시저 호출
EXEC NEW_JOB_PROC('EXAMPLE', 'EXAMPLE', 1000, 1000);
EXEC NEW_JOB_PROC('EXAMPLE'); -- 매개변수가 일치하지 않으면 ERROR
EXEC NEW_JOB_PROC('SAMPLE', 'SAMPLE'); -- DEFALUT 매개변수가 있다면, 기본값으로 전달된다
SELECT * FROM JOBS_IT;

-----------------------------------------------------------------------
-- PLSQL 모든 구문 제어문, 탈출문, 커서 구문을 프로시저에 쓸 수 있다
...
IS
    ...
    SALARY NUMBER; 
BEGIN
    SELECT COUNT(*) -- 값이 없다면 출력 후에 프로시저를 종료, 있다면 값 출력
    INTO CNT
    FROM JOBS 
...
--- 그리고 정상종료라면 commit, 예외라면 롤백 처리하도록 처리하고, 예외처리도 작성해주세요.
CREATE OR REPLACE PROCEDURE DEPTS_PROG 
    ( DEPT_ID IN NUMBER,
      DEPT_NAME IN VARCHAR2,
      FLAG IN VARCHAR2
    )
-- (DEPT_ID IN DEPARTMENTS.DEPARTMENT_ID%TYPE, -- %TYPE : 데이터 타입을 모를 때는 DEPARTMENT_ID의 데이터 타입을 복사
...
-------------------------------------------
EXEC DEPTS_PROG(20, 'ID', 'I');
SELECT * FROM DEPTS_PROG;
...
INSERT INTO SALES(SNO, NAME, TOTAL, PRICE) VALUES(2, '콜드부르', 2, 2000);
INSERT INTO SALES(SNO, NAME, TOTAL, PRICE) VALUES(3, '돌체라떼', 1, 3000);

SELECT SUM(TOTAL * PRICE)
FROM SALES WHERE TO_CHAR(REGDATE, 'YYYYMMDD') = TO_CHAR(SYSDATE, 'YYYYMMDD'); -- 등록되어 있는 날짜를 구해서 오늘 날짜를 구함 
-- 
