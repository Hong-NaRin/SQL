...
-- 만약 시퀀스를 초기화 해야 한다면? 
-- 시퀀스의 증가값을 (-)음수로 만들어서 초기화인 것처럼 쓸 수는 있다
...
-- INDEX 종류는 고유인덱스, 비고유인덱스가 있다
-- UNIQUE한 컬럼에는 (UNIQUE인덱스) 고유 인덱스가 쓰인다
-- 일반 컬럼에는 비고유 인덱스를 지정할 수 있다
-- INDEX는 조회를 빠르게 하지만 DML구문이 많이 사용되는 컬럼은 오히려 성능저하를 부를 수 있다
