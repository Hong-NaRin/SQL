...
-- FRIMARY KEY - 테이블 고유키, 중복X, NULL(X), PK는 테이블에서 1개
-- NOT NULL(NULL을 허용하지 않음)
-- UNIQUE KEY (중복X, NULL값은 허용)
-- FOREIGN KEY (참조하는 테이블의 PK를 넣어 놓은 키, 중복O, NULL값 허용
-- CHECK (컬럼에 대한 제약조건)
...
CREATE TABLE DEPTS(
    ...
    DEPT_DATE DATE          DEFAULT SYSDATES, -- 제약조건은 아니며 (컬럼의 기본값)
    DEPT_PHONE VARCHAR2(30) CONSTRAINT DEPTS_DEPT_PHONE_UK UNIQUE,
    DEPT_GENDER CHAR(1)     CONSTRAINT DEPTS_DEPT_GENDER_CK CHECK(DEPT_GENDER('E', 'M')),
    LOCA_ID NUMBER(4)       CONSTRAINT DEPTS_LOCA_ID_FK REFERENCES LOCATION(LOCATION_ID)
);   
 
INSERT INTO DEPTS(DEPT_NO, DEPT_NAME, DEPT_PHONE, DEOPT_GENDER, LOCA_ID)
...
INSERT INTO DEPTS(DEPT_NO, DEPT_NAME, DEPT_PHONE, DEOPT_GENDER, LOCA_ID)
VALUES (2, 'HONG', '010..', 'F', 1700); -- UNIQUE 제약 위배 ('HONG'중복)

-- 2ND (테이블 레벨 제약조건 정의)
DROP TABLE DEPTS;
CREATE TABLE DEPTS (
    DEPT_NO NUMBER(2),
    DEPT_NAME VARCHAR2(30) NOT NULL, -- NOT NULL은 열레벨로 정의
