...
-- 그룹함수는 일반컬럼이랑 동시에 사용 불가능
SELECT FIRST_NAME, AVG(SALARY) FROM EMPLOYEES; -- 에러
-- 그룹함수 뒤에 OVER()를 붙이면 일반컬럼과 동시에 사용이 가능하다
...
SELECT DEPARTMENT_ID, JOB_ID, 
       ...
       COUNT(*) AS 부서인원수,
       COUNT(*)OVER() AS 전체카운트 -- COUNT(*) OVER()을 사용하면 총 행의 개수를 출력 가능
FROM EMPLOYEES
